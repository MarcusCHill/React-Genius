import React from "react";

function BlackHeader(){
  return(
    <div id="blackHeader">
      <ul>
        <li className="blackHeaderList">
          <a href="">Marcus Hill</a>
        </li>
        <span className="blackHeaderList">|</span>
        <li className="blackHeaderList">
          <a href="https://genius.com/#featured-stories">Featured</a>
        </li>
        <li className="blackHeaderList">
          <a href="https://genius.com/#top-songs">Charts</a>
        </li>
        <li className="blackHeaderList">
          <a href="https://genius.com/#videos">Videos</a>
        </li>
        <li className="blackHeaderList">
          <a href="https://genius.com/#community">Community</a>
        </li>
        <span className="blackHeaderList">|</span>
        <li className="blackHeaderList">
          <a href="https://promote.genius.com/">Promote Your Music</a>
        </li>
        <span className="blackHeaderList">|</span>
        <li className="blackHeaderList">
          <a href="https://www.facebook.com/Genius/" className="blackHeaderIcon">
            <svg src="facebook.svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 9.95 20">
              <path d="M8.09 3.81c-1.4 0-1.58.84-1.58 1.67v1.3h3.35L9.49 11h-3v9H2.33v-9H0V6.88h2.42V3.81C2.42 1.3 3.81 0 6.6 0H10v3.81z"></path>
            </svg>
            </a>
        </li>
        <li className="blackHeaderList">
          <a href="https://twitter.com/Genius" className="blackHeaderIcon">
            <svg src="twitter.svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 16.43">
              <path d="M20 1.89l-2.3 2.16v.68a12.28 12.28 0 0 1-3.65 8.92c-5 5.13-13.1 1.76-14.05.81 0 0 3.78.14 5.81-1.76A4.15 4.15 0 0 1 2.3 9.86h2S.81 9.05.81 5.81A11 11 0 0 0 3 6.35S-.14 4.05 1.49.95a11.73 11.73 0 0 0 8.37 4.19A3.69 3.69 0 0 1 13.51 0a3.19 3.19 0 0 1 2.57 1.08 12.53 12.53 0 0 0 3.24-.81l-1.75 1.89A10.46 10.46 0 0 0 20 1.89z"></path>
            </svg>
          </a>
        </li>
        <li className="blackHeaderList">
          <a href="https://www.instagram.com/genius/" className="blackHeaderIcon">
            <svg src="instagram.svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
              <path d="M10 0c2.724 0 3.062 0 4.125.06.83.017 1.65.175 2.426.467.668.254 1.272.65 1.77 1.162.508.498.902 1.1 1.153 1.768.292.775.45 1.595.467 2.424.06 1.063.06 1.41.06 4.123 0 2.712-.06 3.06-.06 4.123-.017.83-.175 1.648-.467 2.424-.52 1.34-1.58 2.402-2.922 2.92-.776.293-1.596.45-2.425.468-1.063.06-1.41.06-4.125.06-2.714 0-3.062-.06-4.125-.06-.83-.017-1.65-.175-2.426-.467-.668-.254-1.272-.65-1.77-1.162-.508-.498-.902-1.1-1.153-1.768-.292-.775-.45-1.595-.467-2.424C0 13.055 0 12.708 0 9.995c0-2.712 0-3.04.06-4.123.017-.83.175-1.648.467-2.424.25-.667.645-1.27 1.153-1.77.5-.507 1.103-.9 1.77-1.15C4.225.234 5.045.077 5.874.06 6.958 0 7.285 0 10 0zm0 1.798h.01c-2.674 0-2.992.06-4.046.06-.626.02-1.245.15-1.83.377-.434.16-.828.414-1.152.746-.337.31-.602.69-.775 1.113-.222.595-.34 1.224-.348 1.858-.06 1.064-.06 1.372-.06 4.045s.06 2.99.06 4.044c.007.633.125 1.262.347 1.857.17.434.434.824.775 1.142.31.33.692.587 1.113.754.596.222 1.224.34 1.86.348 1.063.06 1.37.06 4.045.06 2.674 0 2.992-.06 4.046-.06.635-.008 1.263-.126 1.86-.348.87-.336 1.56-1.025 1.897-1.897.217-.593.332-1.218.338-1.848.06-1.064.06-1.372.06-4.045s-.06-2.99-.06-4.044c-.01-.623-.128-1.24-.347-1.827-.16-.435-.414-.83-.745-1.152-.318-.34-.71-.605-1.143-.774-.596-.222-1.224-.34-1.86-.348-1.063-.06-1.37-.06-4.045-.06zm0 3.1c1.355 0 2.655.538 3.613 1.496.958.958 1.496 2.257 1.496 3.61 0 2.82-2.288 5.108-5.11 5.108-2.822 0-5.11-2.287-5.11-5.107 0-2.82 2.288-5.107 5.11-5.107zm0 8.415c.878 0 1.72-.348 2.34-.97.62-.62.97-1.46.97-2.338 0-1.827-1.482-3.31-3.31-3.31s-3.31 1.483-3.31 3.31 1.482 3.308 3.31 3.308zm6.51-8.633c0 .658-.533 1.192-1.192 1.192-.66 0-1.193-.534-1.193-1.192 0-.66.534-1.193 1.193-1.193.316 0 .62.126.844.35.223.223.35.526.35.843z"></path>
            </svg>
          </a>
        </li>
        <li className="blackHeaderList youtube">
          <a href="https://www.youtube.com/genius" className="blackHeaderIcon">
            <svg src="youtube.svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20.01 14.07">
              <path d="M19.81 3A4.32 4.32 0 0 0 19 1a2.86 2.86 0 0 0-2-.8C14.21 0 10 0 10 0S5.8 0 3 .2A2.87 2.87 0 0 0 1 1a4.32 4.32 0 0 0-.8 2S0 4.51 0 6.06V8a30 30 0 0 0 .2 3 4.33 4.33 0 0 0 .8 2 3.39 3.39 0 0 0 2.2.85c1.46.14 5.9.19 6.68.2h.4c1 0 4.35 0 6.72-.21a2.87 2.87 0 0 0 2-.84 4.32 4.32 0 0 0 .8-2 30.31 30.31 0 0 0 .2-3.21V6.28A30.31 30.31 0 0 0 19.81 3zM7.94 9.63V4l5.41 2.82z"></path>
            </svg>
          </a>
        </li>
      </ul>
    </div>
  )
}

export default BlackHeader;
